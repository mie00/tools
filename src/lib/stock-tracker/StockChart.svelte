			...
			<div class="flex flex-wrap gap-2">
				{#each timeWindows as timeWindow (timeWindow.id)}
					<button
						...
						class="rounded-lg px-3 py-1 text-sm transition-colors {selectedTimeWindow === timeWindow.id
	...
	{:else if chartData.length > 0}
		<div class="space-y-6">
			...
			<div class="w-full overflow-x-auto">
				<div class="h-64 min-w-full rounded-lg border border-gray-200 bg-gradient-to-b from-blue-50 to-gray-50 p-2 sm:h-80 sm:p-4 lg:h-96">
					<div class="flex h-full items-end justify-between gap-1">
						{#each chartData as point, i (i)}
							{@const height = chartData.length > 1 ? ((point.price - minPrice) / (maxPrice - minPrice)) * 85 + 10 : 50}
							...
							<div
								...
								style="height: {height}%; flex: 1; min-width: {chartData.length > 50 ? '3px' : '8px'}; max-width: 20px;"
							>
								<!-- Tooltip -->
								<div class="absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 transform rounded-lg bg-gray-900 px-3 py-2 text-xs text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 whitespace-nowrap">
			...
			<div class="grid grid-cols-2 gap-4 rounded-lg bg-gray-50 p-4 sm:grid-cols-4">
				<div class="text-center">
					<div class="text-xs uppercase tracking-wide text-gray-500">Period Change</div>
					<div class="text-lg font-semibold {periodChange >= 0 ? 'text-green-600' : 'text-red-600'}">
				...
				<div class="text-center">
					<div class="text-xs uppercase tracking-wide text-gray-500">Period High</div>
					<div class="text-lg font-semibold text-green-600">
						{formatCurrency(highPrice)}
					</div>
				</div>
				<div class="text-center">
					<div class="text-xs uppercase tracking-wide text-gray-500">Period Low</div>
					<div class="text-lg font-semibold text-red-600">
						{formatCurrency(lowPrice)}
					</div>
				</div>
				<div class="text-center">
					<div class="text-xs uppercase tracking-wide text-gray-500">Data Points</div>
			...
			<details class="group">
				<summary class="cursor-pointer list-none">
					<div class="flex items-center justify-between rounded-lg bg-gray-100 p-3 transition-colors hover:bg-gray-200">
						<span class="font-medium text-gray-800">View Raw Data</span>
						<svg class="h-5 w-5 text-gray-600 transition-transform group-open:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						...
						<tbody>
							{#each chartData.slice().reverse() as point (point.fullDate || point.date)}
								<tr class="border-b border-gray-100 hover:bg-gray-50">
									...
									<td class="px-4 py-2 text-right font-medium text-gray-900">{formatCurrency(point.price)}</td>
