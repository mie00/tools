<script lang="ts">
	...
	function openImageInNewWindow(media: MediaData) {
		...
		if (win) {
			win.document.write(
				`<img src="${media.data}" style="max-width: 100%; height: auto;">`
			);
		}
	}
...
>
	...
	<div class="flex items-start justify-between">
		...
		<div class="flex space-x-2 {isFocused ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity">
			{#if !isEditing}
				<button
					on:click={(e) => {
						e.stopPropagation();
		...
		<div class="space-y-3">
			...
			{#if isFocused && topics.length > 1 && !isEditing}
				<div class="mt-3 border-t pt-3">
					<p class="text-xs text-gray-500 mb-2">Move to topic:</p>
