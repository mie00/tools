<script lang="ts">
	...
	function exportDiff() {
		...
		a.href = url;
		a.download = 'json-diff-result.txt';
		a.click();
...
<div class="space-y-6">
	...
	<div class="rounded-lg border border-gray-200 bg-white shadow-sm">
		...
		>
			<span class="font-semibold text-gray-900">⚙️ JSON Diff Configuration</span>
		...
		{#if showConfig}
			<div class="space-y-4 border-t border-gray-200 p-4">
				...
				<!-- Max Depth -->
				<div>
					<label for="max-depth-json" class="mb-2 block text-sm font-medium text-gray-700">
		...
		<div class="rounded-lg border border-gray-200 bg-white shadow-sm">
			...
			<div class="p-4">
				<textarea
					...
					placeholder={`{"name": "Original", "value": 123}`}
		...
		<div class="rounded-lg border border-gray-200 bg-white shadow-sm">
			...
			<div class="p-4">
				<textarea
					...
					placeholder={`{"name": "Modified", "value": 456}`}
					class="h-64 w-full resize-none rounded-md border border-gray-300 px-3 py-2 font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
				></textarea>
			</div>
		</div>
				...
				{:else}
					<div class="max-h-96 space-y-2 overflow-y-auto">
						{#each diffResult as item, index (index)}
							<div
								class="flex items-start gap-3 rounded p-2 text-sm {item.type === 'added'
							...
							>
								<span
									class="font-semibold {item.type === 'added'
										? 'text-green-800'
										: item.type === 'removed'
											? 'text-red-800'
											: item.type === 'changed'
