...
<div class="relative" bind:this={dropdownRef}>
	<input
		...
		value={isDropdownOpen
			? searchQuery
			: value
				? getTimezoneDisplayName(value) + getTimezoneAbbreviation(value)
				: ''}
		{placeholder}
		{disabled}
		...
		>
			...
			{:else}
				{#each filteredTimezones as timezone, index (timezone)}
					<button
						type="button"
						class="w-full px-3 py-2 text-left text-gray-900 hover:bg-gray-100 focus:bg-gray-100 focus:outline-none {index ===
						selectedIndex
					...
					>
						<div class="truncate">
							{getTimezoneDisplayName(timezone)}<span class="text-sm text-gray-500"
								>{getTimezoneAbbreviation(timezone)}</span
							>
						</div>
					</button>
				{/each}
